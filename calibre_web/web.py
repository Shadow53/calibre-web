...
feature_support = {"kobo": bool(services.kobo)}
...
@login_required
def toggle_archived(book_id) -> str:
    is_archived = change_archived_books(book_id, message=f"Book {book_id} archive bit toggled")
    if is_archived:
...
def update_view():
    ...
    try:
        ...
        log.exception("Could not save view_settings: %r %r: %e", request, to_save, ex)
...
def get_languages_json():
    ...
    return json.dumps([{"name": r} for r in entries_start[0:5]])
...
def get_matching_tags():
    ...
    q = q.filter(
        db.Books.authors.any(func.lower(db.Authors.name).ilike("%" + author_input + "%")),
...
def render_formats_books(page, book_id, order):
    if book_id == "-1":
        name = _("None")
        entries, random, pagination = calibre_db.fill_indexpage(
            page, 0, db.Books, db.Data.format is None, [order[0][0]], True, CONFIG.config_read_column, db.Data
        )
...
def publisher_list():
    ...
    if current_user.check_visibility(constants.SIDEBAR_PUBLISHER):
        entries = (
            calibre_db.session.query(db.Publishers, func.count("books_publishers_link.book").label("count"))
        ...
        no_publisher_count = (
            ...
            .filter(db.Publishers.name is None)
            .filter(calibre_db.common_filters())
            .count()
        )
...
def get_cover(book_id, resolution=None):
    ...
    cover_resolution = resolutions.get(resolution)
...
def get_robots():
    try:
        ...
        log.exception("No permission to access robots.txt file.")
        abort(403)
...
def serve_book(book_id, book_format, anyname):
    ...
    if book_format.upper() == "TXT":
        ...
        try:
            rawdata = open(os.path.join(CONFIG.get_book_path(), book.path, data.name + "." + book_format), "rb").read()
...
def change_profile(
    ...
    try:
        if (current_user.role_passwd() or current_user.role_admin()) and to_save.get("password", "") != "":
...
def read_book(book_id: int, book_format: str):
    ...
    for file_ext in ["cbr", "cbt", "cbz"]:
        if book_format.lower() == file_ext:
            ...
            return render_title_template(
                "readcbr.html", comicfile=all_name, title=title, extension=file_ext, bookmark=bookmark
            )
    log.debug("Selected book is unavailable. File does not exist or is not accessible")
    flash(_("Oops! Selected book is unavailable. File does not exist or is not accessible"), category="error")
    return redirect(url_for("web.index"))
